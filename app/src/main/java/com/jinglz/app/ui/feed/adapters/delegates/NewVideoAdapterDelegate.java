...
public class NewVideoAdapterDelegate implements AdapterDelegate<List<VideoItemModel>> {
    ...
    /**
     * Constructs new NewVideoAdapterDelegate with specified viewType, imageLoader and listener.
     ...
     */
    public NewVideoAdapterDelegate(@VideoSectionModel.SectionType int viewType,
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent) {
        return new NewVideoViewHolder(LayoutInflater.from(parent.getContext())
                                              .inflate(R.layout.item_feed_video_new, parent, false));
    }
    ...
    public void onBindViewHolder(@NonNull List<VideoItemModel> items,
        ...
        final NewVideoViewHolder vh = (NewVideoViewHolder) holder;
        final AvailableVideoItemModel item = (AvailableVideoItemModel) items.get(position);
        vh.bind(item);
    }

    public class NewVideoViewHolder extends SectioningAdapter.ItemViewHolder
        ...
        @BindView(R.id.badge_jack_pot) LinearLayout badgeJackPot;
        ...
        public View getTutorialView(@TutorialModel.TutorialType int type) {
            switch (type) {
                case TutorialModel.TUTORIAL_TYPE_DRAWING_TIME:
                    if (item.isShowTimer()) {
                        badgeTime.performClick();
                    }
                    return badgeTime;
                case TutorialModel.TUTORIAL_TYPE_DRAWING:
                    return itemView;
                case TutorialModel.TUTORIAL_TYPE_JACK_POT:
                    return badgeJackPot;
                case TutorialModel.TUTORIAL_TYPE_LEARN_MORE:
                    return item.getInfoLink() != null ? badgeInfo : null;
                case TutorialModel.TUTORIAL_TYPE_TIMER:
                    if (!item.isShowTimer()) {
                        badgeTime.performClick();
                    }
                    return badgeTime;
